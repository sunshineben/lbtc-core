...
public:
     ...
     */
    void AvailableCoins(std::vector<COutput>& vCoins, CTxDestination* paddress, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, bool fIncludeZeroValue=false) const;
    ...
    CAmount GetAddressBalance(const CTxDestination& address);
    bool SelectAddress(CTxDestination& address, CAmount& addressBalance, const CAmount amount, int nMinDepth = 0);
    ...
    std::map<CTxDestination, CAmount> GetAddressBalances(int nMinDepth = 0);
